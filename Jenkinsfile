pipeline {
  ...
  post {
    failure {
      script {
        def message = "${env.CHANGE_AUTHOR} <${env.BUILD_URL}|Build #${env.BUILD_NUMBER}> - FAILED to build. [App: ${env.APP_NAME} | Branch: ${env.CHANGE_BRANCH} | Env: ${env.NODE_ENV} | Version: ${env.APP_VERSION}]"
